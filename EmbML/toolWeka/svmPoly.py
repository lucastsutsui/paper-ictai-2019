...
#vars(vars(binarySMO)['m_kernel'])['m_exponent'] value
m_exponent = vars(vars(new_classifiers[1][0])['m_kernel'])['m_exponent']

#vars(vars(binarySMO)['m_kernel'])['m_lowerOrder'] value
m_lowerOrder = vars(vars(new_classifiers[1][0])['m_kernel'])['m_lowerOrder'] 
...
\t\t\tif (p1 != CLASS_INDEX){\n\
\t\t\t\tresultAux " + ("= fxp_sum(result, fxp_mul(instance[p1], m_AttValues[i][j][(k * (INPUT_SIZE + 1)) + p1]))" if useFxp else\
                    "+= instance[p1] * m_AttValues[i][j][(k * (INPUT_SIZE + 1)) + p1]") + ";\n\
\t\t\t}\n\
\t\t}\n"

if m_lowerOrder:
    funcs += "\t\tresultAux " + (("= fxp_sum(resultAux, " + toFxp(1.0) + ")") if useFxp else "+= 1.0") + ";\n"
if m_exponent != 1.0:
    funcs += "\t\tresultAux = " + ("fxp_pow(resultAux, M_EXPONENT)" if useFxp else "pow(resultAux, M_EXPONENT)") + ";\n"

funcs += "\t\tresult " + ("= fxp_sum(result, fxp_mul(m_class_alpha[i][(j == 0 ? 0 : m_size[i][j - 1]) + k], resultAux))" if useFxp else\
...
for i in range(len(new_classifiers)): 
    m_b_i = []
    for j in range(len(new_classifiers)):
        if j < len(new_classifiers[i]):
    ...
    m_b.append(m_b_i)
decls += "\nconst " + ("FixedNum" if useFxp else "float") + " m_b[" + str(len(m_b)) + "][" + str(len(m_b[0])) + "] = "\
         + str(m_b).replace('[','{').replace(']','}') + ";\n"
...
defs += "#define M_EXPONENT " + (toFxp(m_exponent) if useFxp else str(m_exponent)) + "\n"
